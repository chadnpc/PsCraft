...
class BuildOrchestrator : PsCraft {
  ...
  [bool] Compile() {
    [BuildLog]::WriteHeading("[yellow1]Module type: $($this.ModuleType)[/]")
  ...
  [void] CopyFilesParallel([string[]]$FilePaths, [string]$DestinationPath, [ScriptBlock]$Callback) {
    ...
    $progress.Start([Action[ProgressContext]] {
        ...
        $task = $ctx.AddTask("[grey]Copying ModuleFiles[/]", [ProgressTaskSettings]::new())
  ...
  [int] Run([string[]]$tasks) {
    ...
    try {
      if ('Clean' -in $tasks) { [BuildLog]::WriteHeading('[cyan1]Cleaning[/]'); $this.Clean() }
      if ('Compile' -in $tasks) { [BuildLog]::WriteHeading('[cyan1]Compiling[/]'); $this.Compile() }
      if ('Test' -in $tasks) { [BuildLog]::WriteHeading('[cyan1]Testing[/]'); $this.Test() }
    }
    catch {
      $null
    }
