...
class SystemUtils {
  ...
  static [string] GetMachineArchitecture() {
    if ($ENV:PROCESSOR_ARCHITEW6432 -ne $null) { return $ENV:PROCESSOR_ARCHITEW6432 }
    try {
      ...
      try {
        ...
        if (($Attempts -lt $MaxAttempts) -and -not $cancellationToken.IsCancellationRequested) {
          ...
          if (($elapsedTime.TotalSeconds - $DownloadTimeout) -gt 0 -and -not $cancellationToken.IsCancellationRequested) {
            throw [System.TimeoutException]::new("Failed to reach the server: connection timeout: default timeout is $DownloadTimeout second(s)")
          }
          throw
        }
      }
